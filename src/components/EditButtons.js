...
const EditButtons = ({recordListing, setIsNotEditable, updateRecord, deleteRecord}) => {
  return (
    <div>
    <button onClick={(e) => {updateRecord(recordListing.id, recordListing)}} className="btn btn-warning m-2">Save</button>
    <button onClick={setIsNotEditable} type="button" className="btn btn-secondary m-2">Cancel</button>
    <button onClick={deleteRecord} type="button" className="btn btn-danger m-2">Delete</button>
  </div>
  )
